...
- SICP를 봤다면 알겠지만, 이 책을 아무 준비도 안한상태서 보면 안된다. 이 책은 MIT 6.945 코스에서 쓰인다.
  - SICP가 **MIT 1~2학년**이 주요 타겟이지만
  - SDF는
    - **MIT 석사·박사 학생** 이 주요 타겟
    - 3-0-9, Grad AAGS Level, AI Concentration
    - AAGS : Advanced Graduate Subject

- [개요](https://groups.csail.mit.edu/mac/users/gjs/6.945/index.html)
  - 설계자가 예상하지 못한 용도로도 활용될 수 있는 대규모 소프트웨어 시스템의 설계 및 구현을 위한 개념과 기법을 다룸.
  - 응용 분야에는
    - 컴파일러
    - 컴퓨터 대수 시스템
    - 연역 시스템
    - 그리고 일부 인공지능 응용 프로그램이 포함
  - 학습
    - 목표와 전략을 분리하는 방법
    - 가산 데이터 기반 호출 구현 메커니즘
    - 부분적으로 명시된 엔티티를 다루는 방법
    - 다양한 관점을 관리하는 방법 등
  -  주요 주제
     -  콤비네이터
     -  제네릭 연산
     -  패턴 매칭, 패턴 기반 호출, Rule 시스템, 백트래킹, 의존성, 불확정성, 메모이제이션
     -  제약 조건 전파, 점진적 개선 등
  - **매주 상당한 양의 프로그래밍 과제가 수업의 필수적인 부분**
